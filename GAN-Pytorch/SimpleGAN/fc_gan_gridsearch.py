...
for index, (hlayers, relu_slope, z_dim, lr) in enumerate(hparams):
    print(f'Iteration : {index+1}/{len(hparams)}')
    # architectures
    ...
    writer = SummaryWriter(f"runs/hparams/hlayers_{hlayers}-relu_slope_{relu_slope}-z_dim_{z_dim}-lr_{lr}")
    step = 0
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (real, _) in enumerate(loader):
            ...
            losses_epoch_disc.append(lossD.item())
            ...
            losses_epoch_gen.append(lossG.item())
    ...
    writer.add_hparams(
        {"lr": lr, "hlayers": hlayers, "relu_slope": relu_slope, "z_dim": z_dim},
        {
